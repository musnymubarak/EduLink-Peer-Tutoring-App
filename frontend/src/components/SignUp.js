...
export default function SignUp() {
  ...
  const subjectOptions = [
    "Java", "C++", "C#", "Python", "HTML", "Maths", "Sinhala",
  ];
          ...
          >
            {tabData.map((tab) => (
              <button
                ...
                className={`py-2 px-5 rounded-lg transition-all duration-200 text-lg font-semibold ${field === tab.type ? "bg-yellow-300 text-black" : "text-white"}`}
              >
                {tab?.tabName}
              </button>
            ))}
          ...
          <form className="flex w-full flex-col gap-y-4">
            ...
            {field === "tutor" && (
              <>
                ...
                <label className="w-full">
                  <p className="mb-1 text-[0.875rem] leading-[1.375rem] text-richblack-5">
                    Choose Your Subject <sup className="text-pink-200">*</sup>
                  </p>
                  <select
                    required
